...
public class WebServerTest {
	...
	@Test
	public final void testBindStart() throws IOException {
		InetSocketAddress addr;
		servers[25].start();
		while (true) {
			try {
				addr = new InetSocketAddress(rand.nextInt(65535 - 1024) + 1025);
				servers[25].bind(addr, 100);
				break;
			} catch (IOException e) {
				e.printStackTrace();
			}
	...
	@Test
	public final void testBindStop2() {
		InetSocketAddress addr;
		while (true) {
			try {
				addr = new InetSocketAddress(rand.nextInt(65535 - 1024) + 1025);
				servers[26].bind(addr);
				break;
			} catch (IOException e) {
				e.printStackTrace();
			}
	...
	@Test
	public final void testBindStart2() throws IOException {
		InetSocketAddress addr;
		servers[27].start();
		while (true) {
			try {
				addr = new InetSocketAddress(rand.nextInt(65535 - 1024) + 1025);
				servers[27].bind(addr);
				break;
			} catch (IOException e) {
				e.printStackTrace();
			}
	...
	public final void testGetSocketAddress() {
		assertTrue(servers[28].getSocketAddress().toString().contains("0.0.0.0/0.0.0.0"));
	}
	
	@Test(expected = NullPointerException.class)
	public final void testSetNullState() throws IOException {
		servers[29].setState(null);
	}
}
