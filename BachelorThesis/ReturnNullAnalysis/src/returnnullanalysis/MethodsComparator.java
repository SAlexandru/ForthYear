...
public class MethodsComparator {
	...
	public Stack<Object> compare() {
		List<OuterMethodModel> outerMethods = new ArrayList<OuterMethodModel>(
				ListViewHelper.getInstance().getOuterMethods());
		boolean ok = false;
	...
	public boolean areEqual(Statement m1, Statement m2) {
		if (m1 != null && m2 != null) {
			...
			/*
			 * String
			 * []s1=m1.toString().replace("{","").replace("}","").split("\n");
			 * String
			 * []s2=m2.toString().replace("{","").replace("}","").split("\n");
			 * int l1=s1.length; int l2=s2.length; boolean []ok=new boolean[l1];
			 * int i,j; for (i=0;i<l1;i++) { for (j=0;j<l2;j++) { if
			 * (s1[i].equals(s2[j])) ok[i]=true; } if (ok[i]==false) return
			 * false; } //for (i=0;i<l1;i++) //System.out.println(ok[i]); return
			 * true;
			 */
		}
		return false;
