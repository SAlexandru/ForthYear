...
public class FileParser {
	...
	private IMethod outerMethod; // the method where the if (...==null) is
									// located
	...
	class MyASTVisitor extends ASTVisitor {
		public boolean visit(MethodDeclaration method) {
			if (method.resolveBinding() != null)
				outerMethod = (IMethod) method.resolveBinding()
						.getJavaElement();
			return true;
		}
		...
		public boolean visit(final IfStatement node) {
			node.getExpression().accept(new ASTVisitor() {
				public boolean visit(InfixExpression ex) {
					...
					endingLine = cUnit.getLineNumber(node.getThenStatement()
							.getStartPosition()
					...
					try {
						if (left instanceof NullLiteral) {
							if (right instanceof MethodInvocation) {
								processMethod((MethodInvocation) right, node,
										operator);
							} else if (right instanceof SimpleName) {
								processVariable((SimpleName) right, node,
										operator);
							}
						} else if (right instanceof NullLiteral) {
							if (left instanceof MethodInvocation) {
								processMethod((MethodInvocation) left, node,
										operator);
							} else if (left instanceof SimpleName) {
								processVariable((SimpleName) left, node,
										operator);
							}
						}
					} catch (Exception ex1) {
	...
	private void processMethod(MethodInvocation m, IfStatement node,
		...
		if (javaElem instanceof IMethod)
			Pile.getInstance().addMethod(
					new MetModel((IMethod) javaElem, javaElem.getParent()),
					new OuterMethodModel(outerMethod, lineNumber, endingLine,
							op, node.getThenStatement(), node
									.getElseStatement()));
	}

	private void processVariable(SimpleName var, IfStatement node,
			InfixExpression.Operator op) throws JavaModelException {
		IVariableBinding variableBinding = (IVariableBinding) ((SimpleName) var)
				.resolveBinding();
		IJavaElement javaElem = variableBinding.getJavaElement();
		if (variableBinding.isField()) {
			Pile.getInstance().addVariable(
					new VarModel(javaElem, javaElem.getParent(), true),
					new OuterMethodModel(outerMethod, lineNumber, endingLine,
							op, node.getThenStatement(), node
									.getElseStatement()));
		} else {
			Pile.getInstance().addVariable(
					new VarModel(javaElem, javaElem.getParent().getParent(),
							false),
					new OuterMethodModel(outerMethod, lineNumber, endingLine,
							op, node.getThenStatement(), node
									.getElseStatement()));
