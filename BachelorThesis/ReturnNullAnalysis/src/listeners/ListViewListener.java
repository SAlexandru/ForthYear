...
public void selectionChanged(SelectionChangedEvent event)
    ...
    viewEditorItem.addListener(SWT.Selection, new Listener() {
		...
		String selectedOuterMethod = (String)selection.getFirstElement();
		if (selectedOuterMethod!=null)
		   {
		   int i=0;
		   for (i=0;i<ListViewHelper.getInstance().getOuterMethods().size();i++)
			   if (selectedOuterMethod.equals(ListViewHelper.getInstance().getOuterMethods().get(i).getCustomName()))
				   {
				   openFileInEditor(ListViewHelper.getInstance().getOuterMethods().get(i).getMethod().getCompilationUnit(),
						   ListViewHelper.getInstance().getOuterMethods().get(i).getLineNumber());
				   break;
				   }
		   }
    	}
		...
		public void handleEvent(Event event)
			...
			if (selectedOuterMethod!=null)
			   ...
			   for (i=0;i<ListViewHelper.getInstance().getOuterMethods().size();i++)
				   if (selectedOuterMethod.equals(ListViewHelper.getInstance().getOuterMethods().get(i).getCustomName()))
					   {
					   refactoring.createClass(ListViewHelper.getInstance().getOuterMethods().get(i));
		...
		public void handleEvent(Event event)
			...
			if (selectedOuterMethod!=null)
			   ...
			   for (i=0;i<ListViewHelper.getInstance().getOuterMethods().size();i++)
				   if (selectedOuterMethod.equals(ListViewHelper.getInstance().getOuterMethods().get(i).getCustomName()))
					   {
				   	   new ComparisonResultWindow(new MethodsComparator(ListViewHelper.getInstance().getOuterMethods().get(i)).compare());
				   	   break;
				   	   }
			   }
			}    
