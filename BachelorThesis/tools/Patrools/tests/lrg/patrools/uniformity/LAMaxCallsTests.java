...
public class LAMaxCallsTests extends TestCase {
	...
	public void testLAExternalCase() throws JavaModelException {
		...
		for(int i = 0; i < methodGroup.size(); i++) {
			...
			if(theClient.getName().equals("one")) {
				Assert.assertEquals(1, (Double)theClient.getProperty("max invocations level of abstraction").getValue(),0.01);
			}
			...
			if(theClient.getName().equals("callToThis")) {
				Assert.assertEquals(0, (Double)theClient.getProperty("max invocations level of abstraction").getValue(),0.01);
			}
			if(theClient.getName().equals("callToSpecificMethods")) {
				Assert.assertEquals(0, (Double)theClient.getProperty("max invocations level of abstraction").getValue(),0.01);
			}
			if(theClient.getName().equals("callTotoStringOverriden")) {
				Assert.assertEquals(-1, (Double)theClient.getProperty("max invocations level of abstraction").getValue(),0.001);
			}
			if(theClient.getName().equals("callToequalsNotOverriden")) {
				Assert.assertEquals(-1, (Double)theClient.getProperty("max invocations level of abstraction").getValue(),0.01);
			}
		}
		Assert.assertEquals(1, (Double)theClassWrapper.getProperty("max class invocations level of abstraction").getValue(),0.01);
	}
	...
	public void testLASpecialCase() throws JavaModelException {
		...
		for(int i = 0; i < methodGroup.size(); i++) {
			...
			if(theClient.getName().equals("callViaSuperType")) {
				assertEquals(0,(Double)theClient.getProperty("max invocations level of abstraction").getValue(),0.01);
			}
			if(theClient.getName().equals("callViaSpecificType")) {
				assertEquals(0,(Double)theClient.getProperty("max invocations level of abstraction").getValue(),0.01);
			}
		}
		Assert.assertEquals(0, (Double)theClassWrapper.getProperty("max class invocations level of abstraction").getValue(),0.01);
	}
	...
	public void testLAIntraCase() throws JavaModelException {
		...
		for(int i = 0; i < methodGroup.size(); i++) {
			...
			if(theClient.getName().equals("firstMethod")) {
				assertEquals(1,(Double)theClient.getProperty("max invocations level of abstraction").getValue(),0.01);
			}
		...
		for(int i = 0; i < methodGroup.size(); i++) {
			...
			if(theClient.getName().equals("secondMethod")) {
				assertEquals(0.5,(Double)theClient.getProperty("max invocations level of abstraction").getValue(),0.01);
			}
		}
		Assert.assertEquals(0.5, (Double)theClassWrapper.getProperty("max class invocations level of abstraction").getValue(),0.01);
