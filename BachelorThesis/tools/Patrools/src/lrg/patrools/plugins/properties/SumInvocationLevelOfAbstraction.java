...
public class SumInvocationLevelOfAbstraction extends PropertyComputer {

	public SumInvocationLevelOfAbstraction() {
		super("sum invocations level of abstraction", "sum invocations level of abstraction", "method", "numeric");
	}
	 ...
	 public ResultEntity compute(final AbstractEntityInterface aMethod) {
			 ...
			 public boolean visit(MethodInvocation invocation) {
				 ...
				 if(intersection.size() != 0) {
					 ...
					 if(intersection.size() != 1) {
						 res = (1.0 * staticConcreteCone.size() - 1) / (concreteCone.size() - 1);
					 } else {
						 res = 0;
					 }
					 if(result[1] == 1) {
		 ...
		 if(result[1] != 0) {
			 return new ResultEntity(result[0]);
		 } else {
