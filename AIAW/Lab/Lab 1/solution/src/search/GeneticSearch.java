...
public class GeneticSearch extends Object {
	...
	protected Hashtable<?, ?> operatorFitness = null; // used for operator
														// selection
	 ...
	 *
	 * @param debugOn
	 *            the boolean value for the debug flag
	 */
	...
	public void init() {
		...
		summedFitness = new double[populationSize + 1]; // used by roulette
														// wheel
		replacementSize = populationSize;
		try {

			// do some special one-time things to get setup
			geneticObjectClass = Class.forName(geneticObjectClassName); // load
																		// the
																		// class
			GeneticSearchObject dummy = (GeneticSearchObject) geneticObjectClass
					.newInstance();

			if (operatorFitness == null) {

				// get default operators and fitness values
				operatorFitness = dummy.getOperatorFitness();
			}
	...
	/**
	 * Processes one generation by evaluating each chromosome and insert it into
	 * the population, creating new chromosomes and members using
	...
	public void processOneGeneration() {
		...
		Vector<GeneticSearchObject> newPopulation = createNewMembers(); // generate
																		// new
																		// members
	...
	public void search() {
		...
		while ((numPasses < maxNumPasses) && (maxFitness < fitnessThreshold)) {
			...
			trace("Generation[" + numPasses + "] "
					+ population.elementAt(0).toString() + "\n");
		}
	 ...
	 *
	 * @param aClassName
	 *            the String that represents the fully-specified (package
	 *            prefixed) class name of the genetic objects to be searched
	 */
	 ...
	 *
	 * @param size
	 *            the integer value that represents the size of the population
	 */
	...
	public Vector<GeneticSearchObject> getPopulation() {
		if (population == null) {
			return null;
		} else {
			return (Vector<GeneticSearchObject>) population.clone();
		}
	}
	 ...
	 *
	 * @param size
	 *            the number of members to be replaced
	 */
	 ...
	 *
	 * @param rate
	 *            the double value for the crossover rate
	 */
	 ...
	 *
	 * @param rate
	 *            the double value for the mutation rate
	 */
	...
	/**
	 * Sets table of fitness operators and fitness values that are used during
	 * reproduction.
	 *
	 * @param opFitness
	 *            a Hashtable of fitness operators and fitness values (selection
	 *            probabilities). The sum of the fitness values should equal
	 *            100.
	 */
	public void setOperatorFitness(Hashtable<?, ?> opFitness) {
	...
	/**
	 * Creates and initializes a child genetic object from a chromosome if it is
	 * unique or, if duplicates are allowed, even if it is not unique.
	 *
	 * @param chromosome
	 *            the String representation of the child chromosome
	 *
	...
	public GeneticSearchObject createChild(String chromosome) {
		...
		try {
			child = (GeneticSearchObject) geneticObjectClass.newInstance();
	...
	protected void evaluatePopulation() {
		...
		for (int i = 0; i < populationSize; i++) {
			...
			// returns the fitness value computed the first time. For
			// performance
			// it doesn't make sense to keep recomputing a fixed value.
			rawFitness = ((GeneticSearchObject) population.elementAt(i))
					.computeFitness();
	...
	protected Vector<GeneticSearchObject> createNewMembers() {
		...
		try {
			while (newPopulation.size() < replacementSize) {
				...
				childChromosomes = (String[]) invokeOperator(operatorName,
						parentChromosomes);
				if (childChromosomes != null) { // fertile!

					// create 0, 1 or 2 children (depending if duplicates are
					// allowed)
			...
			System.out
					.println("Error: during creation of next generation " + e);
		}
	 ...
	 *
	 * @param newPopulation
	 *            a Vector of new genetic objects to be added to the population
	 */
	 ...
	 *
	 * @param newMember
	 *            the GeneticSearchObject to insert into the population
	 * @param list
	 *            the Vector that contains the population
	 *
	 */
	protected void insertIntoPopulation(GeneticSearchObject newMember,
			Vector<GeneticSearchObject> list) {
		try {
			...
			if (size == 0) {
				...
				for (int i = 0; i < size; i++) { // insert in middle
					GeneticSearchObject tmpMember = (GeneticSearchObject) list
							.elementAt(i);
			...
			System.out
					.println("insertIntoPopulation exception " + e.toString());
		}
	}
	...
	public GeneticSearchObject rouletteWheelSelection() {
		...
		for (int i = 0; i < populationSize; i++) {
			if (summedFitness[i] >= selectionThreshold) {
				GeneticSearchObject member = (GeneticSearchObject) population
						.elementAt(i);
	 ...
	 *
	 * @param operatorName
	 *            the String name of operator to be invoked
	 * @param parents
	 *            the String array that contains parent chromosomes
	 *
	...
	/**
	 * Given two parent chromosomes, does a single-point crossover, creating two
	 * children chromosomes and mutating bits at the specified rate
	 *
	 * @param parents
	 *            the String array that contains the parent chromosomes from
	 *            which two children chromosomes are created
	 *
	...
	/**
	 * Mutates a single chromosome by doing a bitwise test against the mutation
	 * rate and then rolling the dice to select a new bit value from the
	 * vocabulary.
	 *
	 * @param chromosome
	 *            the String that represents the original chromosome
	 *
	...
	public String mutateChromosome(String chromosome) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (rand < mutationRate) { // see if we should mutate the bit
				...
				bitPos = (int) (rand2 * vocabSize); // now see value we should
													// use
				if (bitPos == vocabSize) {
					bitPos = bitPos - 1;
				}
	 ...
	 * @param parents
	 *            the String array that contains the parent chromosomes from
	 *            which the children are created
	 *
	 * @return the set of children chromosomes created from parent chromosomes
	 */
	...
	/**
	 * Mutates two parent chromosomes, subject to the mutation rate, to create
	 * two children chromosomes.
	 *
	 * @param parents
	 *            the String array that represent the parent chromosomes
	 *
