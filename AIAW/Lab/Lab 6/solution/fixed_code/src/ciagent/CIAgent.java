...
/**
 * The <code>CIAgent</code> class defines the common programming interface
 * and behavior for intelligent agents.
 * <p>
 * The lifecycle of a CIAgent is as follows:<ol>
 ...
 *
 *  Call processCIAgentEvent() to perform synchronous event processing
 *  Call postCIAgentEvent() to perform asynchronous event processing
 *
...
public static final int DEFAULT_SLEEPTIME = 15000;  // 15 seconds
  ...
  protected String name;                              // the agent's name
  private CIAgentState state = new CIAgentState();
  private CIAgentTimer timer = new CIAgentTimer(this);
  transient private Vector<CIAgentEventListener> listeners = new Vector<CIAgentEventListener>();            // list of listeners
  ...
  protected Vector<CIAgent>  children = new Vector<CIAgent>() ;  // contained agents (if any)
   ...
   *
   * @param newName the String that contains the new name of this agent
   */
  ...
  /**
   * Sets the agent sleep time (in milliseconds) for autonomous processing. Note
   * that this has no effect on a running agent.
   *
   * @param sleepTime the sleep time (in milliseconds) for this agent
   */
  ...
  /**
   * Sets the time (in milliseconds) for asynchronous event processing. Note that
   * this has no effect on a running agent.
   *
   * @param asyncTime the time (in milliseconds) for asynchronous event processing
   */
   ...
   *
   * @param traceLevel the integer that represents the trace level
   */
   ...
   *
   * @param agentPlatform the AgentPlatform object for this agent
   */
...
public Vector<CIAgent> getChildren() {
  ...
  *
  *  @param parent the CIAgent object that is the parent of this agent
  */
  ...
  */
  ...
  //    System.out.println("CIAgent: " + name + " dispatching an Async event");
   ...
   *
   * @param event the CIAgentEvent to be posted
   */
   ...
   *
   * @param listener the CIAgentEventListener to be added
   */
  public synchronized void addCIAgentEventListener(CIAgentEventListener listener) {
   ...
   *
   * @param listener the CIAgentEventListener to be removed
   */
  public synchronized void removeCIAgentEventListener(CIAgentEventListener listener) {
   ...
   *
   * @param e the CIAgentEvent to be sent to all listeners
   */
  @SuppressWarnings("unchecked")
protected void notifyCIAgentEventListeners(CIAgentEvent e) {
  ...
  *
  * @param child the CIAgent object to be removed from this agent
  *
  */
   ...
   *
   *
   * @param     ois the ObjectInputStream object from which this object
   *            is to be read
   *
  ...
  private void readObject(ObjectInputStream theObjectInputStream) throws ClassNotFoundException, IOException {
    ...
    listeners       = new Vector<CIAgentEventListener>();
